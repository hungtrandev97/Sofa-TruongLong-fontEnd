...
class StateTogglerStorage {
  ...
  removeState(classname) {
    const data = Storages.localStorage.get(
      StateTogglerStorage.STORAGE_KEY_NAME
    );
    if (data) {
...
const StateToggler = (props) => {
  const prevClickHandler = props.onClick; // save the onClick of previous handler

  const toggle = new StateTogglerStorage();

  const classname = props.state; // the classname to apply/remove
  const { target } = props; // selector where to apply the state
  const noPersist = !!props.nopersist; // avoid saving state to localstorage
